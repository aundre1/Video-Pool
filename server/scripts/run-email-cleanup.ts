...
async function markEmailAsInvalid(userId: string, reason: string): Promise<void> {
  try {
    await db.update(users)
      .set({
        // Since we're using a JSON metadata column
        metadata: sql`JSON_SET(COALESCE(${users.metadata}, '{}'), 
          '$.emailValid', false, 
...
/**
 * Mark an email as valid in the database
 */
async function markEmailAsValid(userId: string): Promise<void> {
  try {
    await db.update(users)
      .set({
        metadata: sql`JSON_SET(COALESCE(${users.metadata}, '{}'), 
    ...
    console.error(`Error marking email as valid for user ${userId}:`, error);
  }
}
...
async function validateEmailsBatch(
  ...
  for (let i = 0; i < userEmails.length; i += batchSize) {
    ...
    await Promise.all(batch.map(async ({ id, email }) => {
      ...
      if (!email || email.trim() === '') {
        await markEmailAsInvalid(id, 'Empty email');
        invalid++;
        syntaxErrors++;
        return;
      }
...
async function cleanEmailDatabase(options = { 
  ...
  // Convert ID to string for our processing function
  const emailsToProcess = userEmails.map(user => ({
    ...user,
    id: String(user.id)
  }));
